...
func (a *Common) generateConfig(dependencies asset.Parents, templateData *bootstrapTemplateData) error {
	...
	etchostsIgnConfig, err := aroign.EtcHostsIgnitionConfig(installConfig.Config.ClusterDomain(), aroDNSConfig.APIIntIP, aroDNSConfig.IngressIP, aroDNSConfig.GatewayDomains, aroDNSConfig.GatewayPrivateEndpointIP)
	if err != nil {
		return err
	}
